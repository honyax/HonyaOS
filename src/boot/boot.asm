...
entry:
		...
		;---------------------------------------
        jmp     ipl                         ; iplへジャンプ
        times   3 - ($ - $$) db 0x90        ; 必要であれば0x90(NOP)で埋める
        db      'HONYAIPL'                  ; 0x03( 8) OEM名
        dw      512                         ; 0x0B( 2) セクタのバイト数
        db      4                           ; 0x0D( 1) クラスタのセクタ数
        dw      49                          ; 0x0E( 2) 予約セクタ数（IPL:8KB, カーネル:16KB, FAT12ヘッダ）
        db      2                           ; 0x10( 1) FAT領域数
        dw      512                         ; 0x11( 2) ルートエントリ数
        dw      1328                        ; 0x13( 2) 総セクタ数16
        db      0xF8                        ; 0x15( 1) メディアタイプ
        dw      1                           ; 0x16( 2) FAT領域のセクタ数
        dw      0x20                        ; 0x18( 2) トラックのセクタ数
        dw      0x40                        ; 0x1A( 2) ヘッド数
        ...
        db      'HONYA-OS   '               ; 0x2B(11) ボリュームラベル
        db      'FAT12   '                  ; 0x36( 8) FATタイプ
