...
void setup_gate_descriptor(int index, int base, ushort selector, byte flags)
{
    idt[index].base_lo = (ushort)(base & 0x0000FFFF);
    idt[index].selector = selector;
    idt[index].reserved = 0;
    idt[index].flags = flags;
    idt[index].base_hi = (ushort)((base & 0xFFFF0000) >> 16);
}
...
void setup_segment_descriptor(int index, int limit, int base, ushort flags)
{
    gdt[index].limit_lo = (ushort)(limit & 0x0000FFFF);
    gdt[index].base_lo = (ushort)(base & 0x0000FFFF);
    gdt[index].base_mid = (byte)((base & 0x00FF0000) >> 16);
    gdt[index].flags = (byte)(flags & 0x00FF);
    gdt[index].limit_hi_flags = (byte)((limit & 0x00F0000) >> 16 | (flags & 0xF000) >> 8);
    gdt[index].base_hi = (byte)((base & 0xFF000000) >> 24);
}

void setup_local_segment_descriptor(int index, int limit, int base, ushort flags)
{
    ldt[index].limit_lo = (ushort)(limit & 0x0000FFFF);
    ldt[index].base_lo = (ushort)(base & 0x0000FFFF);
    ldt[index].base_mid = (byte)((base & 0x00FF0000) >> 16);
    ldt[index].flags = (byte)(flags & 0x00FF);
    ldt[index].limit_hi_flags = (byte)((limit & 0x00F0000) >> 16 | (flags & 0xF000) >> 8);
    ldt[index].base_hi = (byte)((base & 0xFF000000) >> 24);
}
...
void init_descriptor()
{
    ...
    setup_segment_descriptor(GDT_IDX_LDT, sizeof(ldt) - 1, (uint)ldt, GDT_LDT_FLAGS);
...
void create_task(TSS *task, int task_index)
{
    ...
    setup_segment_descriptor(GDT_IDX_ROOT_TSS + task_index, GDT_TSS_LIMIT, (uint) task, GDT_FLAGS_TYPE_TSS);
}
